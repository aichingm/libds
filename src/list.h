...
#define LIST_POP_s(type, list, property_name) \
    ((list)->last != NULL ? LISTITEM_AS_s(type, list_pop(list), property_name) : NULL)

// Remove an item from the end of the list and return a pointer to the struct holding the item using the default item
// property name.
...
#define LIST_SHIFT_s(type, list, property_name) \
    ((list)->first != NULL ? LISTITEM_AS_s(type, list_shift(list), property_name) : NULL)

// Remove an item from the beginning of the list and return a pointer to the struct holding the item using the default
// item property name.
...
#define LIST_GET_s(type, list, index, property_name) \
    ((list_length(list) > index && index >= 0) ? LISTITEM_AS_s(type, list_get(list, index), property_name) : NULL)
 ...
 */
#define LIST_ITER_REVERSE(iter_name, begin) \
    for (listitem_t* iter_name = begin; iter_name != NULL; iter_name = iter_name->prev)
...
bool list_contains(list_t* l, listitem_t* i) { return i->list_ptr == l; }

void list_foreach(list_t* l, void (*iter)(listitem_t*, void*), void* userdata) {
    assert(l != NULL);

    LIST_ITER(elem, l->first) { iter(elem, userdata); }
}

void list_foreach_reverse(list_t* l, void (*iter)(listitem_t*, void*), void* userdata) {
    assert(l != NULL);

    LIST_ITER_REVERSE(elem, l->last) { iter(elem, userdata); }
}
...
void list_to_array(list_t* l, listitem_t** array) {
    ...
    LIST_ITER(elem, l->first) { array[i++] = elem; }
}
...
void list_data_to_array(list_t* l, void** array, size_t offset) {
    ...
    LIST_LENGTH_TYPE i = 0;
    LIST_ITER(elem, l->first) { array[i++] = (void*)(((char*)elem) - ((char*)offset)); }
}
...
bool listitem_in_list(listitem_t* i) { return i->list_ptr != NULL; }
